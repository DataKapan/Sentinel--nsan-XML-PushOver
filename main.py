...
def setup_logging(log_path):
    ...
    # Mevcut handler'ları kaldır ve kapat
    for handler in logging.root.handlers[:]:
        try:
            handler.close()
        except Exception:
            pass # Kapatırken hata olursa yoksay
        logging.root.removeHandler(handler)
    # Yeni handler'ları ayarla
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - [%(filename)s:%(lineno)d] - %(message)s', # Satır numarası eklendi
        handlers=[
            logging.FileHandler(log_path, mode='a', encoding='utf-8'),
            logging.StreamHandler()
        ],
        force=True # Eski handler'lar varsa üzerine yazmaya zorla
    )
    # Diğer kütüphanelerin log seviyesini düşür
...
def log_and_print(message, level=logging.INFO):
    """Mesajı hem dosyaya hem de konsola yazdırır."""
...
class SensorReader:
    def __init__(self, config):
        ...
        if self.presence_pin is not None:
            try:
                GPIO.setmode(GPIO.BCM) # Sadece bir kere ayarla
                GPIO.setup(self.presence_pin, GPIO.IN)
                log_and_print(f"GPIO {self.presence_pin} varlık pini olarak ayarlandı.")
            except Exception as e:
                log_and_print(f"GPIO {self.presence_pin} ayarlanamadı: {e}. Varlık tespiti UART'tan denenek.", logging.ERROR)
                self.presence_pin = None
        else:
             log_and_print("GPIO varlık pini config'de tanımlanmamış. Varlık tespiti UART'tan denenek.", logging.WARNING)

        self.uart_config = {'parity': serial.PARITY_NONE, 'stopbits': serial.STOPBITS_ONE, 'bytesize': serial.EIGHTBITS, 'timeout': 0.1}
        self.calibration_cmd = bytes.fromhex('FDFCFBFA0800120000000064000004030201')
        self.read_cmd = bytes.fromhex('FDFCFBFA0800120000006400000004030201')

    def parse_distance(self, data):
        try:
            data_str = data.decode('ascii', errors='ignore')
            if 'Range' in data_str:
                parts = data_str.split('Range')[-1].strip().split()[0]
                return float(parts)
        except Exception as e: log_and_print(f"Radar veri parse hatası: {e}", logging.WARNING)
        return None

    async def initialize(self):
        try:
            self.ser = serial.Serial(port=self.port, baudrate=self.baudrate, **self.uart_config)
            self.ser.write(self.calibration_cmd)
            log_and_print("Radar kalibrasyon komutu gönderildi.")
            await asyncio.sleep(1)
            if self.ser.in_waiting > 0: self.ser.read(self.ser.in_waiting)
            log_and_print(f"Radar sensörü (UART {self.port} @ {self.baudrate}) başarıyla başlatıldı.")
            return True
    ...
    async def read_presence_and_distance(self):
        presence_from_gpio = False
        distance = None
        current_presence = False

        if self.presence_pin is not None:
             try: presence_from_gpio = bool(GPIO.input(self.presence_pin))
             except RuntimeError: # GPIO cleanup yapıldıysa
                 try: GPIO.setup(self.presence_pin, GPIO.IN); presence_from_gpio = bool(GPIO.input(self.presence_pin)) # Tekrar dene
        ...
        if self.ser:
            try:
                ...
                if self.ser.in_waiting > 0:
                    ...
                    distance = self.parse_distance(received)
                    if distance is not None: self.last_distance = distance
            except serial.SerialException as e:
                log_and_print(f"UART okuma/yazma hatası: {e}. Port tekrar açılıyor...", logging.ERROR)
                self.ser = None # Bağlantıyı resetle
                await asyncio.sleep(1) # Kısa bekleme
                try: # Tekrar bağlanmayı dene
                     self.ser = serial.Serial(port=self.port, baudrate=self.baudrate, **self.uart_config)
                     log_and_print("UART portu başarıyla tekrar açıldı.", logging.INFO)
                except Exception as e_reopen: log_and_print(f"UART portu tekrar açılamadı: {e_reopen}", logging.ERROR)
    ...
    def cleanup(self):
        ...
        # GPIO cleanup program sonunda yapılıyor.
    ...
    async def capture_photo(self, case_id=None):
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                "libcamera-jpeg", "-o", photo_path, "--width", "1280", "--height", "720",
                "--nopreview", "-t", "1000", # Otomatik pozlama için 1sn bekle
...
class FTPQueueManager:
    ...
    def _ensure_ftp_path(self, ftp, path_parts):
        ...
        current_path = ftp.pwd() # Başlangıç yolunu al
        try:
            for part in path_parts:
                ...
                try: ftp.cwd(part)
                    ...
                    except Exception as e_mkd: raise e_mkd
        finally:
            try: ftp.cwd(current_path) # İşlem bitince eski yola dön
            except Exception: pass # Dönemezse sorun değil
...
# ==============================================================================
# PushoverClient Sınıfı (Basitleştirilmiş ve Düzeltilmiş)
# ==============================================================================
class PushoverClient:
    def __init__(self, config):
        ...
        self.client = httpx.AsyncClient(timeout=30.0) if self.enabled else None # Timeout'u biraz artır
    ...
    async def initialize(self):
        ...
        try:
            ...
            if response.status_code == 400 and "user key is invalid" in response.text:
...
class SentinelSystem:
    ...
    async def periodic_internet_check(self):
        ...
        log_and_print(f"Periyodik internet kontrolü başlatıldı (Her {interval} saniyede bir).")
        await asyncio.sleep(15)
        while True:
            is_currently_online = await self.check_internet_connection()
            if is_currently_online != self.is_online:
                self.is_online = is_currently_online; status_message = "Çevrimiçi" if self.is_online else "Çevrimdışı"
                log_and_print(f"İNTERNET DURUMU DEĞİŞTİ: {status_message}", logging.INFO)
                status_update_msg = f"{'🟢' if self.is_online else '🔴'} Cihaz Durumu ({self.device_id}): {status_message}"
                await self.pushover.send_message(status_update_msg, title="Sentinel Bağlantı Durumu")
                if self.is_online: await self.process_pending_notifications()
    ...
    async def initialize(self):
        ...
        if not pushover_ok: log_and_print("Uyarı: Pushover başlatılamadı! Bildirimler gönderilemeyebilir.", logging.WARNING)
        ...
        # Pushover başlatılamadıysa başlangıç mesajı gönderme
        if pushover_ok and self.is_online:
             await self.pushover.send_message(f"🟢 Cihaz Aktif: {self.device_id} (İnsan Tespit Modu)", title="Sentinel Başlatıldı")
        elif not self.is_online:
             log_and_print("İnternet yok, başlangıç bildirimi atlandı.", logging.WARNING)
    ...
    async def process_human_detection(self):
        ...
        # FTP'ye sadece geçerli bir dizin varsa ekle
        if os.path.isdir(current_case_copy['dir']):
             self.ftp_manager.queue_case(current_case_copy['dir'])
        else:
             log_and_print(f"FTP'ye eklenemedi: Vaka dizini bulunamadı {current_case_copy['dir']}", logging.ERROR)
        self.current_case = None # Vakayı hemen kapat
        
        # Bildirimleri gönder veya kuyruğa ekle
        if photo_with_human: # Sadece fotoğraf varsa bildirim gönder/kuyruğa ekle
            if self.is_online:
             ...
             log_and_print(f"İnsan tespiti ({case_id}) için kanıt fotoğrafı yok, bildirim gönderilmedi/kuyruğa eklenmedi.", logging.WARNING)

        log_and_print(f"Vaka {case_id} işlemleri tamamlandı.")

    async def run(self):
        if not await self.initialize():
            log_and_print("Sistem başlatılamadı. Çıkılıyor.", logging.CRITICAL)
        ...
        while True:
            try:
                ...
                if presence and not self.motion_being_processed:
                    ...
                    for i in range(5):
                        ...
                        if photo_path_raw and os.path.exists(photo_path_raw):
                            photo_captured = True
                            # Fotoğrafı listeye eklemeden önce case hala var mı kontrol et
                            if self.current_case: self.current_case['photos'].append(photo_path_raw)
                            else: break # Case kapatıldıysa fotoğrafı işlemeye gerek yok
                                
                            is_human, _ = await self.detector.process_image(photo_path_raw)
                            if is_human:
                                # İnsan bulununca case hala var mı kontrol et (çok nadir yarış durumu olabilir)
                                if self.current_case: human_found = True; await self.process_human_detection(); break
                                else: log_and_print("İnsan bulundu ama vaka zaten kapatılmıştı.", logging.WARNING); break
                        else: # Fotoğraf çekilemedi
                        ...
                        # Eğer insan bulunmadıysa ve son fotoğrafa gelmediysek bekle
                        if not human_found and i < 4: await asyncio.sleep(2)
                    
                    # Döngü bitti veya kesildi. Vaka hala açıksa (insan bulunmadıysa veya kamera hatasıysa) kapat.
                    if not human_found and self.current_case:
                        await self.discard_case()
                ...
                # Olası bir döngü kilitlenmesini önlemek için
                if self.current_case: await self.discard_case() # Açık vaka varsa iptal et
                self.motion_being_processed = False # Kilidi aç
                await asyncio.sleep(10) # Hata sonrası daha uzun bekle
...
if __name__ == "__main__":
    ...
    startup_message = f"SENTINEL GÜVENLİK SİSTEMİ v2.1 (Pushover Group) - Cihaz ID: {CONFIG['DEVICE_ID']}"
    ...
    try:
        ...
        # Loop'u çalıştır (KeyboardInterrupt veya hata olana kadar)
        main_loop.run_until_complete(main_task)

    except KeyboardInterrupt:
        log_and_print("Program kullanıcı tarafından sonlandırıldı (KeyboardInterrupt).")
        # Ana görevi iptal etmeye çalış
        if 'main_task' in locals() and not main_task.done():
            ...
            try:
                ...
                pass # Beklenen iptal hatası
    except SystemExit as e:
        log_and_print(f"Program SystemExit ile sonlandırıldı: {e}")
    except Exception as e:
        log_and_print(f"KRİTİK HATA: Beklenmeyen hata ana döngüden çıktı: {e}\n{traceback.format_exc()}", level=logging.CRITICAL)
    finally:
        ...
        try:
             ...
             if tasks:
                 ...
                 main_loop.run_until_complete(asyncio.sleep(1.0))
                 # Tamamlanmalarını bekle (hata verebilirler)
             ...
             # GPIO cleanup en sonda yapılmalı
             ...
             except NameError: # GPIO hiç import edilmediyse (başlangıçta hata olduysa)
                 pass
        ...
        # Event loop'u kapat (gerekliyse)
        # if main_loop.is_running(): main_loop.stop()
        # if not main_loop.is_closed(): main_loop.close()

        print("\nSistem kapatıldı.");
        logging.shutdown() # Logları dosyaya yazmayı bitir
